-- Fix Atoms embedding dimension to match Gemini (768)
-- 1. Update Table
BEGIN;
  -- We clear existing embeddings because 1536 -> 768 conversion is not mathematically valid for different models
  -- and will cause errors. We assume re-indexing will happen.
  UPDATE "Atoms" SET embedding = NULL;
  ALTER TABLE "Atoms" ALTER COLUMN embedding TYPE vector(768);
COMMIT;

-- 2. Update RPC match_atoms
...
CREATE OR REPLACE FUNCTION match_atoms(
  query_embedding vector(768), -- UPDATED from 1536
