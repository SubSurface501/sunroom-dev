CREATE OR REPLACE FUNCTION match_atoms(
  ...
  filter_universe_ids uuid[] DEFAULT NULL, -- ADDED
  p_storyline_id uuid DEFAULT NULL,  -- RENAMED for consistency
  filter_permanence_types text[] DEFAULT NULL, -- ADDED
...
BEGIN
  ...
  WHERE (a.user_id = query_user_id OR a.project_id = query_project_id)
    ...
    AND (
      ...
      OR (a.storyline_id = p_storyline_id AND a.permanence = 'dynamic')
    )
    -- Additional Filters
