...
CREATE TABLE IF NOT EXISTS persona_profiles (
    ...
    fidelity_weight FLOAT DEFAULT 0.5, -- Added from 028 to keep core complete
...
-- Policies
DROP POLICY IF EXISTS "Users can CRUD their own semantic memories" ON semantic_memories;
CREATE POLICY "Users can CRUD their own semantic memories" ON semantic_memories USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can CRUD their own persona profile" ON persona_profiles;
CREATE POLICY "Users can CRUD their own persona profile" ON persona_profiles USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Authenticated users can read domain concepts" ON domain_concepts;
CREATE POLICY "Authenticated users can read domain concepts" ON domain_concepts FOR SELECT USING (auth.role() = 'authenticated');

DROP POLICY IF EXISTS "Authenticated users can insert domain concepts" ON domain_concepts;
