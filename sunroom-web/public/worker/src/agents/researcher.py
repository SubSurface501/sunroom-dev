...
class ResearcherAgent(BaseAgent):
    ...
    def run_task(self, volume_id: str):
        ...
        # 2. Fetch All Nodes (Trailheads) for this Volume
        try:
        ...
        for node in nodes:
            ...
            local_dossier = self._build_dossier(queries, user_id)
            ...
            if blueprint_id:
                ...
                for conn in connections:
                    if conn.get('to') == blueprint_id:
                        parent_blueprint_id = conn.get('from')
                        break
                
                # 3. If parent found, get its Real UUID and fetch its dossier
                if parent_blueprint_id:
                    parent_real_id = blueprint_map.get(parent_blueprint_id)
                    if parent_real_id and parent_real_id in node_lookup:
                        ...
                        parent_dossier = parent_node.get('content', {}).get('research_dossier', [])

            # Merge: Child definitions override Parent definitions if conflict (by Name)
            final_dossier = self._merge_dossiers(parent_dossier, local_dossier)
            ...
            try:
                self.db.table("Trailheads").update({"content": content}).eq("id", node['id']).execute()
            except Exception as e:
